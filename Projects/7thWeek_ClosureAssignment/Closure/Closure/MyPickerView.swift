...
extension UITextField {
    ...
    func loadDropdownDataEditMode(data: [String], text:String) {
        self.inputView = MyPickerView(pickerData: data, pickerText: text, dropdownField: self)
    }
    
    func loadDropdownData(data: [String], onSelect selectionHandler : @escaping (_ selectedText: String) -> Void) {
        print("Step 0")
        self.inputView = MyPickerView.init(pickerData: data, dropdownField: self, onSelect: selectionHandler)
    }
}

class MyPickerView: UIPickerView, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate {
